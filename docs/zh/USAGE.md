...
开发）。GitHub 仓库（`RegularGuy1998/claude-harness`，公开）用于在其他
机器上安装。`harness-cli` 二进制由该仓库的 GitHub Releases 构建与发布；由于仓库是公开的，
首次会话的下载可匿名进行 —— 无需认证。若存在 `gh` 或 `GH_TOKEN`/`GITHUB_TOKEN`，仍会自动
使用（便于避开 API 速率限制）。用
