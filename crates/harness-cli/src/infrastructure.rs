...
pub trait HarnessRepository {
    ...
    fn capture_context(&self, input: ContextCaptureInput) -> Result<i64>;
...
impl HarnessRepository for SqliteHarnessRepository {
    ...
    fn capture_context(&self, input: ContextCaptureInput) -> Result<i64> {
        let sha256 = match input.sha256 {
            Some(value) => value,
            None => hash_file_sha256(&input.path)?,
        };
        let connection = self.open_existing()?;
        connection.execute(
            "INSERT INTO project_context (kind, path, sha256, summary, notes)
             VALUES (?1, ?2, ?3, ?4, ?5);",
            params![input.kind, input.path, sha256, input.summary, input.notes],
        )?;
...
fn hash_file_sha256(path: &str) -> Result<String> {
...
mod tests {
    ...
    use super::*;
    use crate::application::{
        BacklogAddInput, BacklogCloseInput, ContextCaptureInput, DecisionAddInput, IntakeInput,
        InterventionAddInput, InterventionFilter, StoryAddInput, StoryUpdateInput,
    ...
    #[test]
    fn capture_context_inserts_and_reads_back() {
        let (_temp_dir, repository) = test_repository();
        repository.init().unwrap();
        let id = repository
            .capture_context(ContextCaptureInput {
                kind: "pack".to_owned(),
                path: "docs/context/PROJECT_CONTEXT.md".to_owned(),
                sha256: Some("deadbeef".to_owned()),
                summary: "test pack".to_owned(),
                notes: None,
            })
            .unwrap();
        assert_eq!(id, 1);
        ...
        let table = repository
            .query_sql("SELECT path, sha256 FROM project_context ORDER BY id DESC LIMIT 1")
            .unwrap();
        assert_eq!(table.rows.len(), 1);
        assert_eq!(table.rows[0][0], "docs/context/PROJECT_CONTEXT.md");
        assert_eq!(table.rows[0][1], "deadbeef");
    }

    #[test]
    fn capture_context_autocomputes_sha256_when_omitted() {
        let (temp_dir, repository) = test_repository();
        repository.init().unwrap();
        let file = temp_dir.path().join("PROJECT_CONTEXT.md");
        ...
        let table = repository
            .query_sql("SELECT sha256 FROM project_context ORDER BY id DESC LIMIT 1")
            .unwrap();
        // sha256("hello world")
        assert_eq!(
            table.rows[0][0],
