...
#[derive(Subcommand, Debug)]
enum ContextAction {
    /// Capture or refresh the project context-pack pointer.
    Capture(ContextCaptureArgs),
    /// Show captured context-pack pointer(s). Defaults to the latest.
    Show(ContextShowArgs),
}

#[derive(Args, Debug)]
struct ContextCaptureArgs {
    /// Repo-relative path to the committed context-pack file.
    #[arg(long)]
    path: String,
    /// One-line description of the captured context.
    #[arg(long)]
    summary: String,
    #[arg(long, default_value = "pack", value_name = "pack|note")]
    kind: String,
    /// Precomputed sha256; if omitted, the file at --path is hashed.
    #[arg(long)]
...
struct ContextShowArgs {
    /// Show every captured row instead of only the latest.
    #[arg(long)]
    all: bool,
}
...
enum QueryView {
    ...
    /// Summary counts.
    Stats,
    /// Captured project context-pack pointers.
    Context,
    /// Run arbitrary SQL.
...
pub fn run(cli: Cli) -> Result<(), InterfaceError> {
    ...
    match cli.command {
        ...
        Command::Context(args) => match args.action {
            ContextAction::Capture(args) => {
                let id = service.capture_context(ContextCaptureInput {
                    kind: args.kind,
                    path: args.path,
                    sha256: args.sha256,
                    summary: args.summary,
                ...
                println!("Context #{id} captured.");
            }
            ContextAction::Show(args) => {
                print_query_table(&service.query_sql(&context_query_sql(args.all))?);
            }
        },
        ...
        Command::Query(args) => match args.view {
            ...
            QueryView::Context => print_query_table(&service.query_sql(&context_query_sql(true))?),
            QueryView::Sql { query } => {
                if query.is_empty() {
...
fn context_query_sql(all: bool) -> String {
    let base = "SELECT id, captured_at, kind, path, sha256, summary, notes \
                FROM project_context ORDER BY id DESC";
    if all {
        base.to_string()
    } else {
        format!("{base} LIMIT 1")
